...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.9\textwidth]{impact_N_K.pdf}
...
The journal capacity $K$ acts as the critical constraint on the screening effectiveness. We identify two critical threshold values for $K$ that define the boundaries of the stability regimes. These thresholds are derived analytically by solving for the capacity required to make the expected utility of a bad paper zero ($E[U_{Bad}] = 0$) at the boundary states of the system ($x=0$ and $x=1$).

The required screening probability $\sigma^*$ to neutralize the incentive for cheating is derived from the utility condition:
\begin{equation}
    E[U_{Bad}] = \sigma^* (-D - \lambda B) + (\lambda B - C) = 0 \implies \sigma^* = \frac{\lambda B - C}{D + \lambda B}
\end{equation}
Using the default parameters, we obtain $\sigma^* \approx 0.5385$. The corresponding capacity $K$ is related to the submission volume $V(x)$ by $K = \sigma^* (C_J V(x) + \epsilon)$.

\begin{enumerate}
    \item \textbf{Threshold for Global Honesty ($K_{high}$)}: This occurs when the journal can maintain effective screening ($\sigma \ge \sigma^*$) even under the maximum submission load at full corruption ($x=0, V(0)=N$).
    \begin{equation}
        K_{high} = \sigma^* (C_J N + \epsilon) \approx 269.2
    \end{equation}
    If $K > K_{high}$, the system converges to full honesty regardless of the initial state.

    \item \textbf{Threshold for Global Corruption ($K_{low}$)}: This occurs when the journal fails to maintain effective screening even under the minimum submission load at full honesty ($x=1, V(1)=\alpha N$).
    \begin{equation}
        K_{low} = \sigma^* (C_J \alpha N + \epsilon) \approx 53.9
    \end{equation}
    If $K < K_{low}$, the system collapses to full corruption regardless of the initial state.
\end{enumerate}

The dynamic behavior of the system under these three regimes is illustrated in Figure \ref{fig:K_impact}.

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.9\textwidth]{K_x.pdf}
    \caption{Phase portrait of the replicator dynamics $\dot{x}$ for three distinct capacity regimes. The red dashed line ($K \approx 53.9$) represents a low-capacity scenario leading to global corruption. The blue solid line ($K \approx 99.9$) represents the bistable regime with an internal unstable equilibrium at $x^* \approx 0.786$. The green dash-dotted line ($K \approx 269.2$) represents a high-capacity scenario ensuring global honesty.}
    \label{fig:K_impact}
\end{figure}


It is worth noting that the dynamic curve for the high capacity scenario (Green line in Figure \ref{fig:K_impact}) exhibits a non-smooth "kink" or discontinuity in its derivative. This phenomenon arises due to the saturation of the screening probability.
At low values of $x$ (high congestion), the screening probability is limited by capacity: $\sigma(x) = K / (C_J V(x) + \epsilon) < 1$. However, as the proportion of honest authors $x$ increases, the submission volume $V(x)$ decreases significantly. Once $x$ exceeds a certain threshold, the capacity $K$ becomes sufficient to review every submission thoroughly, causing the calculated $\sigma(x)$ to reach its physical upper bound of $1$.
\begin{equation}
    \sigma(x) = \min\left(1, \frac{K}{C_J V(x) + \epsilon}\right)
\end{equation}
The kink represents the transition point where the system switches from a congestion-constrained state to an uncongested state where $\sigma(x)$ is fixed at 1. Beyond this point, the dynamics are driven solely by the replicator term $x(1-x)$, resulting in a change in the curvature of $\dot{x}$.
...
\begin{enumerate}
    \item \textbf{Global Honesty Regime ($x^* < 0$)}: When $N$ is small (left side of the curve), the calculated tipping point $x^*$ is negative. Mathematically, this implies that for any physically valid state $x \in [0,1]$, the system lies to the "right" of the unstable repeller. Consequently, the evolutionary dynamics always drive the system towards $x=1$. Physically, the population is small enough that the journal can effectively screen all submissions even in the worst-case scenario.
    
    \item \textbf{Global Corruption Regime ($x^* > 1$)}: When $N$ is extremely large (right side of the curve), $x^*$ exceeds 1. The entire physical range $[0,1]$ lies to the "left" of the repeller, causing the system to collapse towards $x=0$. Here, the sheer volume of submissions overwhelms the fixed capacity, making screening ineffective regardless of author behavior.
    
    \item \textbf{Bistability Regime ($0 \le x^* \le 1$)}: For intermediate populations (highlighted in yellow), the unstable equilibrium $x^*$ lies within the physical range. This phenomenon, known as \textbf{Bistability}, indicates that the long-term outcome of the system is not determined solely by the parameters, but is \textit{path-dependent}.
    \begin{itemize}
        \item If the initial proportion of honest authors is high ($x_0 > x^*$), the system generates enough "self-discipline" (low submission volume) to maintain effective screening, converging to Honesty ($x=1$).
        \item If the initial proportion is low ($x_0 < x^*$), the high volume of junk submissions dilutes screening capacity, making cheating profitable and driving the system to Corruption ($x=0$).
    \end{itemize}
\end{enumerate}

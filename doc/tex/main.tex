...
\begin{figure}[htbp]
    ...
    \caption{\textbf{Schematic representation of the Threshold Competition Model.} The process unfolds in three stages: (1) Submission into a pool $S$;
...
Assume the population consists of $N_{OG}$ authors playing $OG$ and $N_{AS}$ authors playing $AS$ (where $N_{OG} + N_{AS} = N - 1$, excluding the focal author).
...
The journal maximizes its utility by choosing a strategy $S_J \in \{OG, AA, OB, AR\}$.
...
Let $S_{good}$ and $S_{bad}$ be the total number of good and bad papers submitted by the authors, such that $S = S_{good} + S_{bad}$.
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Rationing Probability ($\rho_{OG}$):}
    \begin{equation}
        \rho_{OG} = \min\left(1, \frac{K}{S_{good}(1-\epsilon) + S_{bad}\lambda}\right)
    \end{equation}
    \item \textbf{Payoff ($U_J(OG)$):} The journal gains $B$ from good papers and loses $D$ from bad papers, scaled by $\rho_{OG}$.
    \begin{equation}
        U_J(OG) = \rho_{OG} \left[ B \cdot S_{good}(1-\epsilon) - D \cdot S_{bad}\lambda \right] - C(S)
    \end{equation}
\end{itemize}
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Rationing Probability ($\rho_{AA}$):} This strategy maximizes congestion.
    \begin{equation}
        \rho_{AA} = \min\left(1, \frac{K}{S}\right)
    \end{equation}
    \item \textbf{Payoff ($U_J(AA)$):}
    \begin{equation}
        U_J(AA) = \rho_{AA} \left[ B \cdot S_{good} - D \cdot S_{bad} \right] - C(S)
    \end{equation}
\end{itemize}
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Rationing Probability ($\rho_{OB}$):}
    \begin{equation}
        \rho_{OB} = \min\left(1, \frac{K}{S_{good}\epsilon + S_{bad}(1-\lambda)}\right)
    \end{equation}
    \item \textbf{Payoff ($U_J(OB)$):} Although the journal intends to select bad papers, it may accidentally accept good papers due to review errors ($\epsilon$).
    However, it also accepts the vast majority of bad papers.
    \begin{equation}
        U_J(OB) = \rho_{OB} \left[ B \cdot S_{good}\epsilon - D \cdot S_{bad}(1-\lambda) \right] - C(S)
    \end{equation}
    Note that despite the potential small gain from $B \cdot S_{good}\epsilon$, the term $-D \cdot S_{bad}(1-\lambda)$ typically dominates because $D > B$ and $(1-\lambda) \gg \epsilon$ in realistic settings.
    Thus, $U_J(OB)$ remains strictly dominated by strategies that do not actively seek penalties.
\end{itemize}
...
An author chooses a strategy $S_A \in \{OG, OB, AS, NS\}$.
...
Assuming the journal plays the standard \textit{Only Good} strategy (implying the rationing probability is $\rho_{OG}$), the payoffs for the four author strategies are derived as follows:
...
\begin{itemize}
    ...
    \item \textbf{Payoff:}
    \begin{equation}
        U_A(OG) = \alpha \left[ (1-\epsilon) \rho_{OG} r - c \right]
    \end{equation}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Payoff:}
    \begin{equation}
        U_A(OB) = (1-\alpha) \left[ \lambda \rho_{OG} r - c \right]
    \end{equation}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Payoff:}
    \begin{equation}
        U_A(AS) = \left[ \alpha(1-\epsilon) + (1-\alpha)\lambda \right] \rho_{OG} r - c
    \end{equation}
\end{itemize}
...
\begin{equation}
    U_A(OG) = \alpha \left[ (1-\epsilon) \cdot E[\rho_{\text{focal}}] \cdot r - c \right]
\end{equation}
...
They pass review with probability $P_{pass}(AS)$. If they pass, they are accepted with probability $E[\rho_{\text{focal}}]$.
\begin{equation}
    U_A(AS) = \left[ (\alpha(1-\epsilon) + (1-\alpha)\lambda) \cdot E[\rho_{\text{focal}}] \cdot r \right] - c
\end{equation}
...
\begin{equation}
    U_J = B \cdot (E[M_G] \cdot \bar{\rho}) - D \cdot (E[M_B] \cdot \bar{\rho}) - C(E[S])
\end{equation}
...
We first analyze the author's strategy space. The strategy \textit{Never Submit} (NS) serves as the baseline with a fixed payoff of $U_A(NS) = 0$.
...
The expected payoff is $U_A(OB) = (1-\alpha)[\lambda \rho r - c]$.
...
Consequently, $U_A(OB) < U_A(NS)$ holds universally, rendering OB a strictly dominated strategy.
In contrast, the \textit{Only Good} (OG) and \textit{Always Submit} (AS) strategies are conditionally viable.
$U_A(OG)$ remains positive as long as the rationing probability satisfies $\rho > c/[r(1-\epsilon)]$.
...
Thus, the author's effective strategy space reduces to $\{OG, AS, NS\}$.
Next, we examine the journal's strategy space, using \textit{Always Reject} (AR) as the baseline with $U_J(AR)=0$.
The \textit{Only Bad} (OB) strategy, representing an irrational inverse selection policy, yields a payoff of $U_J(OB) = \rho_{OB} [ B S_{good}\epsilon - D S_{bad}(1-\lambda) ] - C(S)$.
...
The penalty term from correctly identifying bad papers overwhelmingly dominates the accidental benefit term, ensuring $U_J(OB) < 0$ for any non-zero submission volume.
Thus, Journal OB is strictly dominated by AR. The \textit{Always Accept} (AA) strategy is conditionally dominated.
Its payoff $U_J(AA)$ becomes negative if the penalty for bad papers satisfies the reputation constraint $D > B (S_{good}/S_{bad})$.
...
Only Bad (OB) & $\rho_{OB}[B S_{good}\epsilon - D S_{bad}(1-\lambda)] - C(S)$ & \textbf{Strictly Dominated} & Dominated by AR. Penalties ($D$) outweigh accidental benefits ($B\epsilon$). \\
Always Accept (AA) & $\rho_{AA}[B S_{good} - D S_{bad}] - C(S)$ & Conditionally Dominated & Dominated by AR if $D > B \frac{S_{good}}{S_{bad}}$. \\
Only Good (OG) & $\rho_{OG}[B S_{good}(1-\epsilon) - D S_{bad}\lambda] - C(S)$ & \textbf{Dominant} & Strictly yields highest utility under standard parameters. \\

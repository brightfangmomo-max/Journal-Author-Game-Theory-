...
\subsection{Players and timing}\label{subsec:players-timing}
We study a one-journal, multi-author publication ecosystem. The player set consists of a single journal
\(J\) and a population of \(N_A \ge 1\) authors \(A=\{1,2,\dots,N_A\}\).
In each round (or submission opportunity), each author independently draws a paper quality
\(q \in \{0,1\}\), where \(q=1\) denotes a \emph{good} paper and \(q=0\) denotes a \emph{bad} paper.
Let
\[
\alpha := \Pr(q=1) \in (0,1)
\]
denote the probability that a randomly produced paper is good.

The journal commits to an editorial rule (strategy) that maps perceived quality into an accept/reject
decision. Authors then choose whether and what to submit according to their submission strategies.

\subsection{Author strategies}\label{subsec:author-strategies}
Each author \(i \in A\) chooses one of four pure strategies:
\[
S_A=\{AS,\,OG,\,OB,\,NS\},
\]
representing extreme submission behaviours under binary research quality:
\begin{itemize}
  \item \(AS\) (\emph{Always Submit}): submit regardless of quality;
  \item \(OG\) (\emph{Only Good}): submit if and only if \(q=1\);
  \item \(OB\) (\emph{Only Bad}): submit if and only if \(q=0\);
  \item \(NS\) (\emph{No Submit}): never submit.
\end{itemize}
Equivalently, the implied proportions of good/bad submissions \((p_g,p_b)\) from a representative author are
\[
(p_g,p_b)=
\begin{cases}
(\alpha,\,1-\alpha), & AS,\\
...
\subsection{Journal strategies}\label{subsec:journal-strategies}
...
\subsection{Binary quality and noisy review}\label{subsec:quality-review}
Paper quality is binary, \(q \in \{0,1\}\).
To allow imperfect peer review, assume the journal observes a noisy signal \(\hat{q}\in\{0,1\}\) with
misclassification errors
\[
\varepsilon := \Pr(\hat{q}=0 \mid q=1) \quad \text{(false rejection / Type I error)}, \qquad
\lambda := \Pr(\hat{q}=1 \mid q=0) \quad \text{(false acceptance / Type II error)}.
\]
Under this formulation, the acceptance probabilities for a truly good/bad paper are
\[
(q_g, q_b) =
\begin{cases}
(1,\,1), & OA,\\
(1-\varepsilon,\,\lambda), & OG,\\
(\varepsilon,\,1-\lambda), & OB,\\
(0,\,0), & NS,
\end{cases}
\]
where \(q_g := \Pr(\text{accept}\mid q=1)\) and \(q_b := \Pr(\text{accept}\mid q=0)\).


\subsection{Notation and parameters}\label{subsec:notation-parameters}
This subsection summarises the key parameters used in the model.

\paragraph{Population and quality generation.}
\begin{itemize}
  \item \(N_A \in \mathbb{N}\): number of authors in the population.
  \item \(q \in \{0,1\}\): true (intrinsic) paper quality, where \(q=1\) is \emph{good} and \(q=0\) is \emph{bad}.
  \item \(\alpha \in (0,1)\): probability that an author produces a good paper in a given round,
  \[
  \alpha := \Pr(q=1), \qquad \Pr(q=0)=1-\alpha.
  \]
\end{itemize}

\paragraph{Peer review (noisy signal).}
\begin{itemize}
  \item \(\hat{q} \in \{0,1\}\): journal's perceived quality signal after review.
  \item \(\varepsilon \in [0,1]\): Type I error (false rejection / false negative),
  \[
  \varepsilon := \Pr(\hat{q}=0 \mid q=1),
  \]
  i.e., a good paper is mistakenly perceived as bad.
  \item \(\lambda \in [0,1]\): Type II error (false acceptance / false positive),
  \[
  \lambda := \Pr(\hat{q}=1 \mid q=0),
  \]
  i.e., a bad paper is mistakenly perceived as good.
\end{itemize}

\paragraph{Editorial rules and acceptance probabilities.}
Given a journal strategy \(s_J \in S_J=\{OA,OG,OB,NS\}\), define:
\begin{itemize}
  \item \(q_g(s_J)\): acceptance probability conditional on a truly good paper \(q=1\),
  \[
  q_g(s_J) := \Pr(\text{accept}\mid q=1, s_J).
  \]
  \item \(q_b(s_J)\): acceptance probability conditional on a truly bad paper \(q=0\),
  \[
  q_b(s_J) := \Pr(\text{accept}\mid q=0, s_J).
  \]
\end{itemize}
Under the binary-signal error structure above,
\[
(q_g(s_J), q_b(s_J)) =
\begin{cases}
(1,\,1), & s_J=OA,\\
(1-\varepsilon,\,\lambda), & s_J=OG,\\
(\varepsilon,\,1-\lambda), & s_J=OB,\\
(0,\,0), & s_J=NS.
\end{cases}
\]

\paragraph{Author strategy set.}
Authors choose \(s_A \in S_A=\{AS,OG,OB,NS\}\), where:
\begin{itemize}
  \item \(AS\): submit regardless of \(q\);
  \item \(OG\): submit iff \(q=1\);
  \item \(OB\): submit iff \(q=0\);
  \item \(NS\): never submit.
\end{itemize}

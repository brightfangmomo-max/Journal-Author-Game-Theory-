...
T_TOTAL    = 3000
A_MAX      = 0.80
# Start just above initial tipping threshold (~0.80) — system is in the
# honesty basin but with little safety margin.
X0              = 0.82
COLLAPSE_THRESH = 0.30

# Two community sizes:
#   N_LARGE = 1000  (full field, original parameters)
#   N_SMALL =  200  (academic sub-field; capacities scaled proportionally)
N_LARGE     = 1000
N_SMALL     =  200
BATCH_LARGE =   50   # 20 batches of 50
BATCH_SMALL =   20   # 10 batches of 20

np.random.seed(42)
N = N_LARGE   # used in ODE / threshold helpers

# ── AI-extended effective parameters ─────────────────────────────────────────
def K_rev_eff(a):
    """Net screening capacity: rises with delta but is diluted by gamma > delta."""
...
def Pi_bad_ode(x, a):
    ...
    lam  = lam_eff(a)
    c    = c_eff(a)
    V    = N * (1 - (1 - ALPHA) * x)          # volume after volume scaling absorbed into Krev
...
ode_traj = ode_trajectory()

# Tipping threshold curve
thresh_curve = np.array([find_threshold(a) or np.nan for a in a_series])

# ── AI-extended ABM (extending run_batch_abm from ABM_batch_matching.py) ─────
def run_abm_with_ai(a_schedule, x0=X0, N_pop=N_LARGE, batch_size=BATCH_LARGE, beta=BETA):
    """
    Extends run_batch_abm: at each step the effective K_rev, lambda, and c
    are updated according to the current AI adoption level a_t.
    The volume scaling (1+gamma*a) is absorbed into K_rev_eff so that the
    local screening intensity automatically reflects the AI-driven congestion.
    """
    n_steps = len(a_schedule)
    scale   = batch_size / N_pop

    # Initialise population (capacities scaled to N_pop)
    ...
    for t in range(n_steps):
        ...
        # Update AI-dependent parameters (scale to N_pop then to batch)
        ...
        # Batch-matching interaction phase
        ...
        # Social learning: 2 % of population revise per step (Fermi rule)
...
# ── Run 30 replications for each community size ───────────────────────────────
...
# ── ODE critical threshold (first a where x*(a) rises above x0) ──────────────
a_crit_ode = None
...
# Individual small-community runs
...
plt.savefig('doc/tex/fig9_abm_validation.pdf', bbox_inches='tight')
print("fig9 saved.")
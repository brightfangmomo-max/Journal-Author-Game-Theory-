---
name: validate-bib
description: Validate bibliography entries against citations in all paper files. Find missing entries and unused references.
allowed-tools: ["Read", "Grep", "Glob"]
---

# Validate Bibliography

Cross-reference all citations in paper files against bibliography entries.

## Steps

1. **Read the bibliography file** (`Bibliography.bib`) and extract all citation keys

2. **Scan all paper files for citation keys:**
   - `.tex` files: look for `\cite{`, `\citet{`, `\citep{`, `\citeauthor{`, `\citeyear{`
   - Extract all unique citation keys used

3. **Cross-reference:**
   - **Missing entries:** Citations used in papers but NOT in bibliography
